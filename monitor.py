...
class Monitor:
    ...
    def _average_total_distance(self):
        ...
        for driver in self._activities[DRIVER]:
            for activity in range (len(self._activities[DRIVER][driver])-1):
                total_distance += (manhattan_distance
                                    (self._activities[DRIVER][driver]
                                    [activity].location,
                                    self._activities[DRIVER][driver]
                                    [activity+1].location))
            total_drivers += 1
        if total_distance == 0:
            ...
            return total_distance/total_drivers
    ...
    def _average_ride_distance(self):
        ...
        for driver in self._activities[DRIVER]:
            for activity in range(len(self._activities[DRIVER][driver])):
                if self._activities[DRIVER][driver][activity].description is PICKUP:
                    pickup_location = (self._activities[DRIVER][driver][activity].location)
                    dropoff_location = (self._activities[DRIVER][driver][activity + 1].location)
                    total_distance += manhattan_distance(pickup_location,dropoff_location)
            total_drivers += 1
        if total_distance == 0:
            return 0
        else:
...
if __name__ == '__main__':
    dic = Monitor()
    dic.notify(3, RIDER, REQUEST, 'sohaib', Location(1,2))
    dic.notify(5, DRIVER, REQUEST, 'ben', Location(3,4))
    dic.notify(6, RIDER, PICKUP, 'sohaib', Location(1,2))
    dic.notify(6, DRIVER, PICKUP,'ben', Location(1,2))
    dic.notify(9, DRIVER, DROPOFF, 'ben', Location(6,8))
    dic._average_total_distance()
