...
class DriverRequest(Event):
    ...
    def do(self, dispatcher, monitor):
        ...
        # TODO
        monitor.notify(self.timestamp, DRIVER, REQUEST, self.driver.identifier
...
class Cancellation(Event):
    """
    a rider cancels his request.
    """
    def __init__(self, timestamp, rider):
        """
        initialize a cancellation event at time timestamp of the rider
    ...
    def do(self, dispatcher, monitor):
        """
        Cancels the the request the rider makes for a driver
    ...
    def __str__(self):
        ...
        @rtype: str
        """
        '{} -- {}: Cancels request'.format(self.timestamp, self.rider)

class Pickup(Event):
    """
    Sets the driver's location to the rider's location.
    """
    ...
    def do(self, dispatcher, monitor):
        ...
        monitor.notify(self.timestamp, DRIVER, PICKUP, self.driver.id, self.driver.destination)
        events = []
        travel_time = self.driver.start_time(self.rider.destination)
        ...
        if self.rider.status == WAITING:
            ...
            events.append(RiderRequest(self.timestamp, self.rider))
            self.rider.destination = None
    ...
    def __str__(self):
        ...
        if self.events==[]:
            "There exists no rider to be picked."
        else:
            "{} picks up {} at {}.".format(self.driver,self.rider, self.timestamp)


class Dropoff(Event):
    """
    A driver successfully drops of a rider
    """
    def __init__(self, timestamp, driver):
        """
        Initialize a Dropoff event at time timestamp when driver successfully
        drops a rider
    ...
    def do(self, dispatcher, monitor):
        ...
        @rtype: List[events]
        """
        monitor.notify(self.timestamp, DRIVER, DROPOFF, self.driver.identifier,
    ...
    def __str__(self):
        ...
        @rtype: str
        """
        '{} -- {}: Drops Rider'.format(self.timestamp, self.driver)
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if event_type == "DriverRequest":
                ...
                rider=Rider( rider_id,origin_location,destination_location,Status, paitence)
